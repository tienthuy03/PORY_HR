...
const TVSList3 = ({
    ...
    useEffect(() => {
        //Neu dataItemCurrent khac null thi lay dataItemCurrent nguoi lai lay dataItem[0]
        if (dataItemCurrent) {
            if (dataItemCurrent.code_nm !== undefined && dataItemCurrent.code !== undefined) {
                setSelectName(dataItemCurrent.code_nm);
                setSelectPK(dataItemCurrent.code);
                setColorSelect(null)
                onChangeSelect(dataItemCurrent);
            }
            if ((dataItemCurrent.code_nm == '' && dataItemCurrent.code == '')
                || (dataItemCurrent.code_nm == undefined && dataItemCurrent.code == undefined)
                || dataItemCurrent.code_nm == undefined || dataItemCurrent.code == undefined
            ) {
                setSelectName(titleModal);
                setColorSelect("#B2B2B2")
            }
        }
    }, [dataItemCurrent, dataItem]);
