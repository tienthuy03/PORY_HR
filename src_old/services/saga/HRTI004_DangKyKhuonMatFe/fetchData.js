...
function* LayDanhSachNhanVien() {
  ...
  const org_pk = yield select(
    state => state.HRTI004_DangKyKhuonMatFeReducer.ChonPhongBan.code,
  );
  const param = {
    pro: 'SELHRTI0041100',
    in_par: {
  ...
  if (rs.results === 'S') {
    yield put(action.HRTI004LoadDanhSachNhanVienS([]));
    yield put(action.HRTI004LoadDanhSachNhanVienS(rs.data.data));
  } else {
    Alert.alert('Thông báo', 'Tải dữ liệu thất bại.', [{text: 'Xác nhận'}]);
    yield put(action.HRTI004LoadDanhSachNhanVienF());
  }
}
...
function* LayDanhSachPhongBan() {
  ...
  if (rs.results === 'S') {
    if (org_pk === 0) {
      yield put(
        action.HRTI004ChonPhongBan({
          code: rs.data.data[0].code,
          code_nm: rs.data.data[0].code_nm,
        }),
      );
    }
    yield put(action.HRTI004LoadDuLieuPhongBanS(rs.data.data));
  } else {
    Alert.alert('Thông báo', 'Tải dữ liệu thất bại.', [{text: 'Xác nhận'}]);
    yield put(action.HRTI004LoadDuLieuPhongBanF());
  }
}
...
function* CheckAllEmployee() {
  ...
  yield put(action.HRTI004LoadDanhSachNhanVienS(arrTemp));
}
function* LayDanhSachMayChamCong() {
  ...
  if (rs.results === 'S') {
    ...
    yield put(action.HRTI004LoadDanhSachMayChamCongS(temp));
  } else {
    Alert.alert('Thông báo', 'Tải dữ liệu thất bại.', [{text: 'Xác nhận'}]);
    yield put(action.HRTI004LoadDanhSachMayChamCongF());
  }
}
export {
  LayDanhSachNhanVien,
