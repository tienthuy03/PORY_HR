...
export default function DangKy({}) {
  ...
  const [dataViTriAnTaiCho, setDataViTriAnTaiCho] = useState([]);
  ...
  const [modalVisibleEmpEdit, setModalVisibleEmpEdit] = useState(false);
  ...
  const getData = () => {
    ...
    sysFetch(
      ...
      .then((rs) => {
        console.log("rs SELHRRI005000", rs);
        if (rs == "Token Expired") {
        ...
        if (rs != "Token Expired") {
          if (rs.results == "S") {
            ...
            if (rs.data.lst_org.length > 0) {
              setPhongBan_code(rs.data.lst_org[0].code);
              setPhongBan_code_nm(rs.data.lst_org[0].code_nm);
            }

            setDataViTriAnTaiCho(rs.data.lst_canteen);
            ...
            setLimitDate(rs.data.lst_hint[0].limit_dt);
  ...
  const OnSave = () => {
    ...
    if (dataIns.length === 0) {
      ...
      const processChunk = async (chunkIndex) => {
        ...
        const lst_thr_emp_pk = chunkData.map((item) => item.emp_pk).join("|");
        const lst_from_dt = chunkData
          .map((item) => moment(item.from_dt, "DD/MM/YYYY").format("YYYYMMDD"))
          .join("|");

        const lst_position_canteen_edit = chunkData
          .map((item) => item.position_canteen_edit_code)
          .join("|");
        const lst_method = chunkData.map((item) => item.method_code).join("|");
        const lst_meal_shift = chunkData
          .map((item) => item.meal_shift_code)
          .join("|");
        const lst_reason = chunkData.map((item) => item.reason).join("|");
        const lst_note = chunkData.map((item) => item.note).join("|");

        const pro = "UPDHRRI005000";
        const in_par = {
          p1_varchar2: "INSERT",
          p2_varchar2: lst_thr_emp_pk,
          p3_varchar2: sequence_dt,
          p4_varchar2: phongBan_code,
          p5_varchar2: lst_position_canteen_edit,
          p6_varchar2: lst_method,
          p7_varchar2: lst_meal_shift,
          p8_varchar2: lst_from_dt,
          p9_varchar2: lst_reason,
          p10_varchar2: lst_note,
          p11_varchar2: chunkData.length,
  ...
  return (
    <Block style={{ flex: 1 }} backgroundColor={Color.backgroundColor}>
              ...
              >
                <View>
                  <View style={{ flexDirection: "row" }}>
                    ...
                    >
                      <Text>Phương thức ăn điều chỉnh</Text>
                    </View>
                    <View
                      style={{
                        flexDirection: "row",
                    ...
                    {dataEmployeeIns.map((item) => (
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            <Text>{item.position_canteen_edit_code_nm}</Text>
                          </View>
      ...
      <ModalChonNhanVien
        ...
        dataCaAn={dataCaAn}
        dataLoai={dataLoai}
        dataViTriAnTaiCho={dataViTriAnTaiCho}
        dataViTriAnDongHop={dataViTriAnDongHop}
        warning={warning}
      ...
      <ModalEditNhanVien
        isShow={modalVisibleEmpEdit}
        handleShow={setModalVisibleEmpEdit}
        dataCaAn={dataCaAn}
        dataLoai={dataLoai}
        dataViTriAnTaiCho={dataViTriAnTaiCho}
        dataViTriAnDongHop={dataViTriAnDongHop}
