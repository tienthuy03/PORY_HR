...
const DS = ({}) => {
  ...
  const updateExperience = (sequence_dt, tco_org_pk) => {
    const pro = "UPDHRRI007000";
    const in_par = {
      p1_varchar2: "DELETE",
      p2_varchar2: "",
      p3_varchar2: sequence_dt,
      p4_varchar2: tco_org_pk,
      p5_varchar2: "",
      p6_varchar2: "",
      p7_varchar2: "",
      p8_varchar2: "",
  ...
  const comfirmAlert = (sequence_dt, tco_org_pk) => {
    Alert.alert(
      "Thông báo",
      "Bạn có muốn xoá dữ liệu đăng ký nhận cơm hộp không?",
      [
        {
          text: "Hủy bỏ",
        ...
        {
          text: "Xác nhận",
          onPress: async () => updateExperience(sequence_dt, tco_org_pk),
        },
      ],
      { cancelable: false }
    );
      ...
      <Block flex marginLeft={10} marginBottom={10} marginRight={10}>
        ...
        >
          ...
          >
            <TVSButton
              onPress={() => {
                navigation.navigate("ChiTietNhanComHop", {
                  item: item,
            ...
            {item["del_yn"] == "Y" && (
              <TVSButton
                onPress={() => comfirmAlert(item.sequence_dt, item.tco_org_pk)}
                type={"danger"}
                icon={"delete-forever"}
                buttonStyle={"3"}
              >
