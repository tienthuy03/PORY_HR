...
export default function DangKy({}) {
  ...
  const [about_days_register, setAbout_days_register] = useState(
      ...
      .then((rs) => {
        ...
        if (rs != "Token Expired") {
          if (rs.results == "S") {
            ...
            setObjWarning(rs.data.warning[0]);

            setWarning(rs.data.warning[0].note);
            setLimitDate(rs.data.warning[0].limit_dt);
            setAbout_days_register(rs.data.warning[0].about_days_register);
            setFromDate(rs.data.warning[0].first_day_of_month);
            setToDate(rs.data.warning[0].last_day_of_month);
          }
        }
      })
      .catch((error) => {
  ...
  const OnSave = () => {
    ...
    if (dataIns.length === 0) {
      ...
      const processChunk = async (chunkIndex) => {
        ...
        const lst_rice_rec_pk = chunkData.map((item) => item.pk).join("|");
        const lst_thr_emp_pk = chunkData
          .map((item) => item.thr_emp_pk)
          .join("|");
        ...
        const lst_description = chunkData
          .map((item) => item.description)
          .join("|");

        const pro = "UPDHRRI008000";
        const in_par = {
          p1_varchar2: "INSERT",
          p2_varchar2: lst_rice_rec_pk,
          p3_varchar2: lst_thr_emp_pk,
          p4_varchar2: sequence_dt,
          p5_varchar2: phongBan_code,
          p6_varchar2: lst_reason,
          p7_varchar2: lst_from_dt,
          p8_varchar2: lst_to_dt,
          p9_varchar2: lst_description,
          p10_varchar2: dataIns.length,
  ...
  const onResetData = () => {
    ...
    setFromDate(objWarning.first_day_of_month);
    setToDate(objWarning.last_day_of_month);
  };
  ...
  return (
    <Block style={{ flex: 1 }} backgroundColor={Color.backgroundColor}>
      ...
      <ModalChonNhanVien
        ...
        limitDate={limitDate}
        about_days_register={about_days_register}
        to_code={to_code}
