...
export default function DK() {
  ...
  const OnGetGridData = (code, fdate, tdate) => {
    console.log(code, ",", fdate, ",", tdate);
    const pro = "SELHRRI010001";
    const in_par = {
  ...
  const OnSave = () => {
    ...
    if (dataGrid.length > 0) {
      dataGrid.forEach(function (item) {
        ...
        name += item.name + "|";
        vegetarian += item.vegetarian_dt + "|";
        count++;
      });
    }
    const pro = "UPDHRRI010000";
    const in_par = {
      p1_varchar2: "INSERT",
      p2_varchar2: "",
      p3_varchar2: name,
      p4_varchar2: vegetarian,
      p5_varchar2: num1,
      p6_varchar2: num2,
      p7_varchar2: num3,
      p8_varchar2: num4,
      p9_varchar2: selectCodeObject,
      p10_varchar2: selectCodeMealMethod,
      p11_varchar2: selectCodeCanteen,
      p12_varchar2: moment(fromDate, "DD/MM/YYYY").format("YYYYMMDD"),
      p13_varchar2: moment(toDate, "DD/MM/YYYY").format("YYYYMMDD"),
      p14_varchar2: textCustomer,
      p15_varchar2: textDescription,
      p16_varchar2: thr_emp_pk,
      p17_varchar2: selectCodePaidCost,
      p18_varchar2: selectCodeOrg,
      p19_varchar2: count,
      p20_varchar2: "",
      p21_varchar2: "",
      p22_varchar2: "",
      p23_varchar2: "",
      p24_varchar2: inputEmpID,
      p25_varchar2: inputFullNm,
      p26_varchar2: APP_VERSION,
      p27_varchar2: crt_by,
    };
    const out_par = {
        ...
        <View style={{ flexDirection: "row" }}>
          ...
          <View style={{ flex: 1 }}>
            ...
            {/* <TVSList2
              required={true}
              label={"Trả phí"}
              dataItem={dataPaidCost}
              titleModal={"Chọn"}
              code={selectCodePaidCost}
              code_nm={selectNamePaidCost}
              onChangeSelect={(val) => onChangePaidCost(val)}
              defaultSelected={true}
              disabled={disablePaidCost}
            /> */}
          </View>
        </View>
        {isShowCustomer && (
              ...
              <View>
                {dataGrid.map((item) => (
                  ...
                  >
                    ...
                    >
                      <Text>{item.vegetarian_dt_cv}</Text>
                    </View>
                    {isShow1 == "Y" && (
                      <View
                        style={{
