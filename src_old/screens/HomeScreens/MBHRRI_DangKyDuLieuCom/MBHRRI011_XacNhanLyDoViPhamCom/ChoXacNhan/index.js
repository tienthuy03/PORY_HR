...
const ChoXacNhan = ({ data, onReload }) => {
  ...
  const DsNPD = useSelector(
    (state) => state.HRRI011_XacNhanLyDoViPhamComReducer.DsNPD
  );
  ...
  const [dataDsCXN, setDataDsCXN] = useState([]);
  const getData = () => {
    ...
    sysFetch(
      ...
      .then((rs) => {
        ...
        if (rs != "Token Expired") {
          if (rs.results == "S") {
            setDataDsCXN(rs.data.rtn_value_cxn);
          } else {
  ...
  const changeValue = (pk, text) => {
    ...
    setDataDsCXN(arr);
    // dispatch(HRRI011LayDanhSachCXNS(arr, dataNPD));
  };
  ...
  const onChangeVP2 = (pk, result) => {
    ...
    setDataDsCXN(arr);
    // dispatch(HRRI011LayDanhSachCXNS(arr));
