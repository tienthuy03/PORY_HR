...
const LS = ({ onCallbackSetDate, startDate, endDate }) => {
  ...
  const updateExperience = (actionByPk, meal_shift) => {
  ...
  const comfirmAlert = (actionByPk, meal_shift) => {
    Alert.alert(
      "Thông báo",
      "Bạn có muốn xoá dữ liệu nhận cơm chay không?",
      [
        {
          text: "Hủy bỏ",
        ...
        {
          text: "Xác nhận",
          onPress: async () => updateExperience(actionByPk, meal_shift),
        },
      ],
      { cancelable: false }
    );
      ...
      <Block flex marginLeft={10} marginBottom={10} marginRight={10}>
        ...
        >
          ...
          <Block flex row justifyCenter marginTop={5}>
            <TVSButton
              onPress={() => {
                navigation.navigate("ChiTietNhanComChay", {
                  item: item,
            ...
            <TVSButton
              onPress={() => comfirmAlert(item.pk, item.meal_shift)}
