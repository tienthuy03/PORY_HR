...
const CT = ({ navigation: { goBack }, route }) => {
  ...
  const [dateCaAn1, setDateCaAn1] = useState(
    moment(new Date().getTime()).format("DD/MM/YYYY")
  );
  const [dateCaAn3, setDateCaAn3] = useState(
    moment(new Date().getTime()).format("DD/MM/YYYY")
  );
  const [fromDateCaAn1, setFromDateCaAn1] = useState("");
  ...
  useEffect(() => {
    setFromDate(item.start_dt);
    setAnChayYN1(item.vegetarian1_yn);
    setAnChayYN2(item.vegetarian2_yn);
    setAnChayYN3(item.vegetarian3_yn);
    OnCheckVegetarian(moment(item.start_dt, "DD/MM/YYYY").format("YYYYMMDD"));
    dataLyDo.map((itemm) => {
      if (itemm.code == item.reason) {
        setLyDo_code(itemm.code);
        setLyDo_code_nm(itemm.code_nm);
      }
    });

    dataPart.map((itemm) => {
      if (itemm.code == item.pos_meal) {
        setPart_code(itemm.code);
        setPart_code_nm(itemm.code_nm);
      }
    });

    dataPositionCanteen.map((itemm) => {
      if (itemm.code == item.new_canteen) {
        setPositionCanteen_code(itemm.code);
        setPositionCanteen_code_nm(itemm.code_nm);
      }
    });

    dataEatingShift.map((itemm) => {
      if (itemm.code == item.meal_shift) {
        setCaAn1_code(itemm.code);
        setCaAn1_code_nm(itemm.code_nm);
        setToDate(item.start_dt);
        setDateCaAn1(item.start_dt);
      }

      if (item.meal_shift2 != "" && itemm.code == item.meal_shift2) {
        setSwitchValue(true);
        setCaAn2_code(itemm.code);
        setCaAn2_code_nm(itemm.code_nm);
        setDateCaAn2(item.end_dt);
      }

      if (item.meal_shift3 != "" && itemm.code == item.meal_shift3) {
        setSwitchValue2(true);
        setCaAn3_code(itemm.code);
        setCaAn3_code_nm(itemm.code_nm);
        setDateCaAn3(item.end_dt);
      }
    });
    setFromCaAn1_code(item.meal_shift_old1_code);
    setFromCaAn1_code_nm(item.meal_shift_old1_code_nm);
    setFromDateCaAn1(item.start_dt);
    setGhiChu(item.description);
  }, [item, dataLyDo, dataPositionCanteen, dataEatingShift]);
  ...
  const handleConfirmFromDate = (val) => {
    ...
    setFromDateCaAn1(moment(val).format("DD/MM/YYYY"));
    setDateCaAn1(moment(val).format("DD/MM/YYYY"));
      ...
      .then((rs) => {
        ...
        if (rs != "Token Expired") {
          if (rs.results == "S") {
            ...
            console.log(
              "rs.data.lst_meal_shift",
              rs.data.lst_meal_shift_default
            );
  ...
  const onSave = () => {
    ...
    const in_par = {
      ...
      p7_varchar2: moment(fromDateCaAn1, "DD/MM/YYYY").format("YYYYMMDD"),
    ...
    sysFetch(
      ...
      .then((rs) => {
        ...
        if (rs.results == "F") {
          ...
          if (rs.data.status != null) {
            dialogNoti("Cập nhật thành công");
            onRefresh();
          } else {
            dialogNotif("Cập nhật thất bại");
          }
        }
      })
      .catch((error) => {
    ...
    <Block flex>
      <TVSHeader goBack={goBack}>
        ...
        {setHeaderChil2(
          ...
          "MBHRRI004",
          menuReducer.data.data.menu.filter((x) => x.menu_cd === "MBHRRI004")[0]
            .p_pk
        )
          .toString()
          .toLowerCase()}
      </TVSHeader>
      <Block flex paddingTop={5} paddingBottom={10}>
                ...
                >
                  <Block
                    style={{
                      ...
                      marginHorizontal: 2,
                      marginTop:
                        dateCaAn1 == vegetarianDt || dateCaAn1 == vegetarianDt2
                          ? 3
                          : 0,
                    }}
                  >
                  ...
                  <Block style={{ flex: 1, marginHorizontal: 2 }}>
                    <Block style={styles.titleText}>
                      {dateCaAn1 == vegetarianDt ||
                      dateCaAn1 == vegetarianDt2 ? (
                    ...
                    <TVSDate colorText={"#B2B2B2"} date={dateCaAn1} />
                  </Block>
