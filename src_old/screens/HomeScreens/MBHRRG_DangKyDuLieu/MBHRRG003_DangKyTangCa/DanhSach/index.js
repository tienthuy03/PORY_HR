...
const DanhSach = ({ onCallbackSetDate, startDate, endDate }) => {
  ...
  try {
    ...
    console.log("error MBHRRE010_Vang_V2 Danh sách");
    console.log(error);
  }
  ...
  const getData = (fromday, enday) => {
    sysFetch(
      ...
      .then((rs) => {
        // console.log("rsssssss Danh sach dang ky vang ", rs);
        if (rs == "Token Expired") {
        ...
        if (rs != "Token Expired") {
          if (rs.results == "S") {
            ...
            setData(rs.data.ls_dktc);
            setLengthDataProps(rs.data.ls_dktc.length);
          }
        }
      })
      .catch((error) => {
    ...
    return (
      <Block flex marginLeft={10} marginBottom={10} marginRight={10}>
        ...
        >
          ...
          <Block flex alignCenter marginVertical={10}>
            <TVSButton
              onPress={() => comfirmAlert(item.thr_reg_pk)}
