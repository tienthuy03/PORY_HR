...
import ChoPheDuyet from "../ApproveTabV2/ChoPheDuyet";
import DaPheDuyet from "../ApproveTabV2/DaPheDuyet";
import HetHan from "../ApproveTabV2/HetHan";
import KhongPheDuyet from "../ApproveTabV2/KhongPheDuyet";
...
const PheDuyet = ({ navigation: { goBack } }) => {
  ...
  const getDataBinding = () => {
    ...
    sysFetch(
      ...
      .then((rs) => {
        console.log("rs SELHRAP001201", rs.data.data[0]);
        if (rs == "Token Expired") {
        ...
        if (rs != "Token Expired") {
          ...
          setCurrentDateDisp(rs.data.data[0].current_dt_disp);
          setCurrentDate(rs.data.data[0].current_dt);
  ...
  const fetchData = () => {
    ...
    sysFetch(
      ...
      .then((rs) => {
        console.log("rs SELHRAP012000 ", rs);
        setflagReload(flagReloads++);
        if (rs == "Token Expired") {
          refreshNewToken("fetchData");
        }
        if (rs != "Token Expired") {
          ...
          if (rs.results === "F") {
            Alert.alert("Thông báo", "Lỗi: " + rs.errorData, [
              { text: "Đóng" },
            ]);
          }
          setflagReload(0);
    ...
    <Block flex backgroundColor={Color.backgroundColor}>
      <TVSHeader goBack={goBack}>
        {setHeaderChil2(
          ...
          dataMenuMBHRs,
          "MBHRAP012",
          dataMenuMBHRs.filter((x) => x.menu_cd === "MBHRAP012")[0].p_pk
        )}
      </TVSHeader>
