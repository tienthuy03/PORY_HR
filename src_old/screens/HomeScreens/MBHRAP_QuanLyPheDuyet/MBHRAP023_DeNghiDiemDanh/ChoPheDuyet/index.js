...
const ChoPheDuyet = ({ onReload, data, dataOT, dataInfo, flagReload }) => {
  ...
  const UpdateApproveNote = (pk, value) => {
    ...
    newLst = newLst.map((obj) => {
      if (obj.pk === pk) {
        return { ...obj, checked: "Y", approve_approveNote: value };
      } else {
        return obj;
      }
    });
      ...
      <Block flex marginLeft={10} marginRight={10} marginBottom={5}>
        ...
        >
          ...
          <View style={{ marginTop: 10 }}>
            ...
            <TextInput
              value={item.approve_approveNote}
              onChangeText={(e) => {
  ...
  const OnSave = () => {
    ...
    if (arrData.length > 0) {
      arrData.forEach(function (item) {
        ...
        lstapproveNote +=
          item.approve_approveNote == undefined
            ? ""
            : item.approve_approveNote + "|";
      });
      Alert.alert(
        "Thông báo",
        ...
        [
          ...
          },
          {
            text: "Xác nhận",
            onPress: () => {
              const pro = "UPDHRAP023000";
              const in_par = {
                p1_varchar2: l_action,
                p2_varchar2: lstStatus,
                p3_varchar2: lstPK,
                p4_varchar2: lstDt,
                p5_varchar2: lstapproveNote,
        ...
        if (obj == "OnSave") {
          ...
          if (arrData.length > 0) {
            arrData.forEach(function (item) {
              ...
              lstDt += item.detail_dt + "|";
              lstStatus += item.status + "|";
              lstapproveNote += item.approve_approveNote + "|";
            });
            sysFetch(
              API,
              {
