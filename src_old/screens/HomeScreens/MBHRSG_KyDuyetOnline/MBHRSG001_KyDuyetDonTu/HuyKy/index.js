...
import NoiDungHuyKy from './NoiDungHuyKy';
...
const HuyKy = () => {
    ...
    const getData = () => {
        ...
        sysFetch(
            ...
            .then(res => {
                console.log("Data: ", res.data.huyKy);
                setArrLoaiVanBan(res.data.loaiVanBan);
                setData(res.data.huyKy);
            })
            .catch(error => {
                console.log('error');
                console.log(error);
    ...
    const renderItem = ({ item }) => {
        return (
            <TouchableOpacity
                onPress={() => {
                    navigation.navigate('NoiDungHuyKy', {
                        item: item,
                ...
                <Block backgroundColor={Color.gray} >
                    <Block backgroundColor={Color.white}
                        ...
                        {Object.entries(item).map((oneField) => {
                            ...
                            if (fieldName === 'ngày từ chối' || fieldName === 'người trình ký' || fieldName === 'lý do') {
                                return (
                                    <OneField
                                        value={oneField[1]}
                                        keyName={
