...
const CT = ({ navigation: { goBack }, route }) => {
  ...
  const getData2 = () => {
    ...
    sysFetch(
      ...
      .then((rs) => {
        console.log("rs SELHRRE022001", rs);
        if (rs == "Token Expired") {
          refreshNewToken("getData");
        }
        if (rs != "Token Expired") {
  ...
  const onSave = () => {
    ...
    sysFetch(
      ...
      .then((rs) => {
        console.log("rs UPDHRRE022000", rs);
        if (rs.results == "F") {
          let errors = "";
          try {
    ...
    <Block flex>
      <TVSHeader goBack={goBack}>
        ...
        {setHeaderChil2(
          ...
          "MBHRRE022",
          menuReducer.data.data.menu.filter((x) => x.menu_cd === "MBHRRE022")[0]
            .p_pk
        )
          .toString()
          .toLowerCase()}
