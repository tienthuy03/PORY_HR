...
const LS = ({ onCallbackSetDate, startDate, endDate }) => {
  ...
  try {
    ...
    console.log("error MBHRRE025_DangKyDieuDongXe Danh sách");
    console.log(error);
  }
  ...
  const updateExperience = (seq, emp_pk) => {
      ...
      .then((rs) => {
        ...
        if (rs != "Token Expired") {
          if (rs.results === "S" && rs.data.status === "1") {
            ...
            dialogNoti("Xóa không thành công!");
          }
        }
      })
      .catch((error) => {
  ...
  const comfirmAlert = (seq, thr_emp_pk) => {
    Alert.alert(
      "Thông báo",
      ...
      [
        ...
        {
          text: "Xác nhận",
          onPress: async () => updateExperience(seq, thr_emp_pk),
        },
      ],
      { cancelable: false }
    );
    ...
    return (
      <Block flex marginLeft={10} marginBottom={10} marginRight={10}>
        ...
        >
          ...
          <Block row flex justifyCenter={"center"} marginTop={5}>
            <TVSButton
              onPress={() => comfirmAlert(item.sequence_dt, item.thr_emp_pk)}
