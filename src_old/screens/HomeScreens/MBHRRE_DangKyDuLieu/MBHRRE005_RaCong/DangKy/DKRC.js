...
const DKRC_MBHRRE005 = ({ toggle }) => {
  ...
  const {
    ...
    limitDate,
  } = useSelector((state) => state.HRRE005_RaCongReducer);
  ...
  const onResetForm = () => {
    ...
    dispatch(
      HRRE005SetLyDoRaCong({ code_nm: "Chọn lý do ra cổng", code: "0" })
    );
  ...
  const postData = () => {
    ...
    const p_reason_type = SelectLyDoRaCong.code;
    const p_out_type = SelectedVeViec.code;
    ...
    const p_crt_by = crt_by;
    const p_start_date = oneStartDate == "yyyymmdd" ? "" : oneStartDate;
    // const p_end_date = moment(toDate, 'DD/MM/YYYY').format('YYYYMMDD');
    const p_end_date = oneEndDate == "yyyymmdd" ? "" : oneEndDate;

    NetInfo.fetch().then(() => {
      const pro = "UPDHRRE005000";
      const in_par = {
        p1_varchar2: "INSERT",
        p2_varchar2: "", //p_table_pk
        p3_varchar2: p_thr_emp_pk,
        p4_varchar2: p_reason_type,
        p5_varchar2: p_out_type,
        p6_varchar2: switchValue ? "Y" : "N",
        p7_varchar2: p_from_date,
        p8_varchar2: p_to_date,
        p9_varchar2: p_start_time,
        p10_varchar2: switchValue ? "" : p_start_date,
        p11_varchar2: p_end_time,
        p12_varchar2: switchValue ? "" : p_end_date,
        p13_varchar2: p_description,
        p14_varchar2:
          approveInfo.length > 0 ? currentSelectedPerson.approve_role_type : "",
        p15_varchar2:
          approveInfo.length > 0 ? currentSelectedPerson.thr_emp_pk : "",
        p16_varchar2: p_crt_by,
        p17_varchar2: APP_VERSION,
        p18_varchar2: "",
        p19_varchar2: "",
        p20_varchar2: "",
      };
  ...
  const validate = () => {
    if (SelectLyDoRaCong.code === "0") {
      dialogError("Vui lòng chọn lý do ra cổng.");
      return;
    }
    if (SelectedVeViec.code === "0") {
      dialogError("Vui lòng chọn về việc ra cổng.");
      return;
    }
    if (fromDate === "dd/mm/yyyy") {
    ...
    Alert.alert(
      "Thông báo",
      "Bạn có muốn đăng ký ra cổng hay không?",
      [
        {
          text: "Đóng",
            ...
            >
              <ScrollView bounces={false} showsVerticalScrollIndicator={false}>
                ...
                >
                  <Block flex paddingLeft={5} paddingRight={5} marginBottom={4}>
                    <Block row paddingBottom={3}>
                      <Text paddingLeft={5}>Lý do ra cổng</Text>
                      <Text color={Color.red}> *</Text>
                    </Block>
                    <Button
                      nextScreen={() => {
                        dispatch(HRRE005ShowPopupLyDo());
                      }}
                      row
                      flex
                    ...
                    >
                      ...
                      >
                        <Text
                          color={SelectLyDoRaCong.code === "0" ? "#ccc" : ""}
                        >
                          {SelectLyDoRaCong.code_nm}
                        </Text>
                      </Block>
                ...
                >
                  <Block flex paddingLeft={5} paddingRight={5} marginBottom={4}>
                    <Block row paddingBottom={3}>
                      <Text paddingLeft={5}>Về việc</Text>
                      <Text color={Color.red}> *</Text>
                    </Block>
                    <Button
                      nextScreen={() => {
                        dispatch(HRRE005ShowPopupVeViec(true));
                      }}
                      row
                      flex
                      justifyContent={"space-between"}
                    ...
                    >
                      <Block
                        ...
                        paddingVertical={12}
                      >
                        <Text color={SelectedVeViec.code === "0" ? "#ccc" : ""}>
                          {SelectedVeViec.code_nm}
                        </Text>
                      </Block>
                      <Block justifyCenter paddingRight={10}>
                        <Icon
                ...
                >
                  <Button nextScreen={showDatePickerStart} column flex>
                    <Block row marginBottom={4}>
                      <Text paddingLeft={5}>
                        {switchValue ? "Từ ngày" : "Ngày làm việc"}
                      </Text>
                      <Text color={Color.red}> *</Text>
                    </Block>
                    <Block
                ...
                {switchValue ? (
                  ...
                  >
                    ...
                    >
                      <Block row marginBottom={4}>
                        <Text paddingLeft={5} paddingTop={2}>
                          Từ thời gian
                ...
                <Block row justifyCenter alignCenter paddingTop={10}>
                  <Text flex paddingLeft={10} height={60}>
                    Ghi chú
                  </Text>
                </Block>
                <Block backgroundColor={Color.gray} margin={5} radius={7}>
                  <TextInput
                    padding={10}
                    backgroundColor={Color.gray}
                    borderRadius={6}
                    ...
                    multiline={true}
                    onChangeText={(text) =>
                      dispatch(HRRE005SetDescription(text))
                    }
                  />
                </Block>
                {/* approve info start */}
