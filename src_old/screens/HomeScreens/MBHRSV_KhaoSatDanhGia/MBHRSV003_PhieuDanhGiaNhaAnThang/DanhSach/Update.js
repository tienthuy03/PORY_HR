...
const UpdatePDG = ({ route }) => {
  ...
  const onSend = () => {
    ...
    console.log("upd itemRoute ", itemRoute);
    console.log("upd itemRoute ", month);
    console.log("update ", itemRoute.survey_month);
    sysFetch(
      API,
      {
        pro: "UPDHRSV003000",
  ...
  const bindingData = () => {
    setMonth(itemRoute.survey_month);
    setDateString(itemRoute.month);
  ...
  const [date, setDate] = useState(
    moment(new Date().setMonth(itemRoute.getmonth))
  );
  const [dateString, setDateString] = useState(itemRoute.month);
  const [month, setMonth] = useState(itemRoute.survey_month);
