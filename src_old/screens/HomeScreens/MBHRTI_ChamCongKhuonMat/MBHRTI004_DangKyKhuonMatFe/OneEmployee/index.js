...
const OneEmployee = ({item, onCheck, sortImage}) => {
  ...
  let dsNhanVien = useSelector((state) =>
    state.HRTI004_DangKyKhuonMatFeReducer.DanhSachNhanVien.filter((x) =>
  ...
  const onRegisterFace = () => {
    dispatch(HRTI004ChonNhanVien(item));
    dispatch(HRTI004ShowPopupDangKyKhuonMat(true));
  };
  return (
