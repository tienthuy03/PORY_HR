...
const ChamCongKhuonMat = ({ navigation: { goBack } }) => {
    ...
    return (
      <>
        ...
        >
          ...
          >
            <Text style={{ color: Color.grayPlahoder }}>
              Ngày giờ chấm công:{" "}
            </Text>
          ...
          >
            ...
            >
              ...
              >
                ...
                <Text style={{ flex: 1 }}>{item.emp_id}</Text>
              </View>
              <View
                style={{
                  flexDirection: "row",
              ...
              >
                ...
                <Text style={{ flex: 1 }}>{item.id_num}</Text>
              </View>
              <View
                style={{
                  flexDirection: "row",
              ...
              >
                <Text style={{ color: Color.grayPlahoder }}>Họ tên: </Text>
              ...
              >
                <Text style={{ color: Color.grayPlahoder }}>Phòng ban: </Text>
                <Text style={{ flex: 1 }}>{item.org_name}</Text>
              </View>
            </View>
            <View
              style={{
            ...
            >
              {item.has_image == "Y" ? (
                <>
                  <Text style={{ color: Color.grayPlahoder }}>Hình ảnh</Text>
                  <Image
                    style={{
                      resizeMode: "stretch",
                      width: "100%",
  ...
  const onResetForSearch = (l1, l2, l3, l4, l5, l6) => {
    setDataRecord([]);
    setIsLoading(false);
    setOffset(0);
    setIsListEnd(false);
    onRequestToServer(0, l1, l2, l3, l4, l5, l6);
  };
  const onRequestToServer = (
    ...
    try {
      if (!isLoad && !isEnd) {
        if (offsets == 0) dispatch(ShowGlobalLoading);
        setIsLoading(true);
  ...
  const getDataControl = () => {
    ...
    sysFetch(
      ...
      .then((rs) => {
        ...
        if (rs != "Token Expired") {
          console.log(rs.data);
          setDataOrg(rs.data.data_org);
          setDataType(rs.data.data_type);
        }
      })
      .catch((error) => {
        console.error("error form");
  ...
  const onCallBack = (data) => {
    ...
    if (data.isFilter) {
      ...
      console.log(startDate, endDate, data.org, data.type);
      onResetForSearch(startDate, endDate, data.org, data.type, false, false);
    } else {
      console.log("close");
    }
      ...
      <Block flex backgroundColor={Color.gray} paddingTop={5}>
        ...
        {dataRecord.length > 0 ? (
          <FlatList
            ...
            onEndReached={() =>
              onRequestToServer(offset, p1, p2, p3, p4, isLoading, isListEnd)
            }
            onEndReachedThreshold={0.5}
            refreshing={false}
            onRefresh={() => {
              onResetForSearch(p1, p2, p3, p4, false, false);
            }}
          />
        ) : null}
      </Block>
