...
const DanhSach = ({goDetail}) => {
    ...
    const [countDaKiemXong, setCountDaKiemXong] = useState(0);

    const [dataCanThucHien, setDataCanThucHien] = useState({data: [], tab: ""});
    const [dataDaHoanThanh, setDataDaHoanThanh] = useState({data: [], tab: ""});
    const [dataDaHuy, setDataDaHuy] = useState({data: [], tab: ""});
    const [dataDangThucHien, setDataDangThucHien] = useState({data: [], tab: ""});
    const [dataDaKiemXong, setDataDaKiemXong] = useState({data: [], tab: ""});
    ...
    const [indexDaKiemXong, setIndexDaKiemXong] = useState(0);
    const [indexChuaHoanThanh, setIndexChuaHoanThanh] = useState(0);
    ...
    const [flagEndDataDaKiemXong, setFlagEndDataDaKiemXong] = useState(false);
    const [flagEndDataChuaHoanThanh, setFlagEndDataChuaHoanThanh] = useState(false);
    ...
    const handleLoadMore = (result) => {
        if(!flagEndDataCanThucGien){
            if(result.tab == 'CanThucHien'){
                ...
                fetchData(fromDate, toDate, result.index, 0, 0, 0, 0, 0, );
            }
        }
        if(!flagEndDataDaHoanThanh){
            if(result.tab == 'DaHoanThanh'){
                setIndexDaHoanThanh(result.index);
                fetchData(fromDate, toDate, 0, result.index, 0, 0,0, 0);
            }
        }
        if(!flagEndDataDaHuy){
            if(result.tab == 'DaHuy'){
                setIndexDaHuy(result.index);
                fetchData(fromDate, toDate, 0, 0, result.index, 0, 0, 0);
            }
        }
        if(!flagEndDataDangThucHien){
            if(result.tab == 'DangThucHien'){
                setIndexDangThucHien(result.index);
                fetchData(fromDate, toDate, 0, 0, 0, result.index, 0, 0);
            }
        }
        if(!flagEndDataDaKiemXong){
            if(result.tab == 'DaKiemXong'){
                setIndexDaKiemXong(result.index);
                fetchData(fromDate, toDate, 0, 0, 0, 0, result.index, 0);
            }
        }
        if(!flagEndDataChuaHoanThanh){
            if(result.tab == 'ChuaHoanThanh'){
                setIndexChuaHoanThanh(result.index);
                fetchData(fromDate, toDate, 0, 0, 0, 0, 0, result.index);
            }
        }
    };
    ...
    const fetchData = (fromDate, toDate, 
        ...
        const in_par = {
            ...
            p5_varchar2: indexDaKiemXong ? indexDaKiemXong : 0,
            p6_varchar2: indexChuaHoanThanh ? indexChuaHoanThanh : 0,

            p7_varchar2: from_dt,
            p8_varchar2: to_dt,

            p9_varchar2: jobName,
            p10_varchar2: currentLoaiCongViec.code ? currentLoaiCongViec.code : "",
            p11_varchar2: currentTinhTrang.code ? currentTinhTrang.code : "",
            p12_varchar2: currentDuAn.pk ? currentDuAn.pk : "",
            p13_varchar2: currentMucDoUuTien.code ? currentMucDoUuTien.code : "",
            ...
            .then((rs) => {
                ...
                if (rs != "Token Expired") {
                    if (rs.results == "S") {
                        ...
                        if(rs.data.dataDaKiemXong.length == 0){
                            setFlagEndDataDaKiemXong(true);
                        }
                        if(rs.data.dataChuaHoanThanh.length == 0){
                            setFlagEndDataChuaHoanThanh(true);
                        }
                        ...
                        const dkxData = rs.data.dataDaKiemXong;
                        ...
                        if(isFirstLoad.current){
                            ...
                            setCountDaKiemXong(
                                rs.data.dataDaKiemXong[0] && rs.data.dataDaKiemXong[0].max_item ?
                                rs.data.dataDaKiemXong[0].max_item : 0
                            );
                        ...
                        if(indexDaKiemXong == 0){
                            setDataDaKiemXong({
                                data: dkxData,
                                tab: "DaKiemXong",
                            });
                        }else{
                            const dataDaKiemXongTemp = [...dataDaKiemXong.data, ...dkxData]
                            setDataDaKiemXong({
                                data: dataDaKiemXongTemp,
                                tab: "DaKiemXong",
                            })
                        }
                    }
